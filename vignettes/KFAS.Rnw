...
\begin{eqnarray}\label{ssgeneral}
...
$\alpha_{t+1}$ is $m \times 1$ and $\eta_{t}$ is $k \times 1$ vector. We also denote $\alpha = (\alpha'_1,\ldots,\alpha'_n)'$ and similarly $y = (y'_1,\ldots,y'_n)'$.
...
\begin{eqnarray}\label{ssuniv}
y_{t,i} &=& Z_{t,i}\alpha_{t,i} + \epsilon_{t,i},  \quad i=1,\ldots,p_t, \quad
t=1,\ldots,n, \nonumber\\
...
\pkg{KFAS} uses this univariate approach in all cases. Although $K_t=P_t Z_t'=\COV(a_t,y_t|y_t-1,\ldots,y_1)$, $v_t$, and $F_t$ differ from the standard multivariate versions, we get $a_t=a_{t,1}$ and $P_t=P_{t,1}$ by using the univariate approach. If standard multivariate matrices $F_t$ and $K_t$ are needed for inference, they can be computed later from the results of the univariate filter. As $F_{\ast,i,t}$, $K_{\ast,i,t}$, and $P_{\ast,t}$ coincide with the
...
In practice expression \eqref{eg} suffer from the fact that $w_i= p(y|\theta^i)/g(y|\theta^i)$ is numerically unstable; when number of observations is large, the discrete probability mass function $p(y|\theta^i)$ tends to zero, even when the Gaussian density function $g(y|\alpha^i)$ does not. Therefore it is better to redefine the weights as 
\[
...
\begin{eqnarray*}
\log \hat L(y)
&=& \log L_g(y) +\log \hat w + \log\frac{1}{N}\sum_{i=1}^Nw^*_i,
\end{eqnarray*}
...
\begin{eqnarray}\label{structural}
...
For non-Gaussian models the observation equation \eqref{structural} is replaced by $p(y_t|\theta_t)$ where $\theta_t = \mu_t + \gamma_t + c_t$. Additional Gaussian noise term $\epsilon_t$ can also be included in $\theta_t$ using \code{SSMcustom} function (this is illustrated in Section \ref{illustration})
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%% Comparison to INLA is removed from vignette as INLA is not available from CRAN
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
P_{t+1} &=& T_t (P_t -K_tF^{-1}_tK_t')T_t' + R_t Q_t R_t,
\end{eqnarray*}
...
K_{t,i} &=& P_{t,i} Z_{t,i}' \\
a_{t,i+1} &=& a_{t,i} + K_{t,i}F_{t,i}^{-1} v_{t,i} \\
P_{t,i+1} &=& P_{t,i} -K_{t,i}K'_{t,i}F_{t,i}^{-1}\\
a_{t+1,1} &=& T_t a_{t,p_t+1}\\
P_{t+1,1} &=& T_t P_{t,p_t+1} T_t' + R_t Q_t R_t, 
\end{eqnarray*}
...
\begin{eqnarray*}
v_{t,i} &=& y_{t,i} - Z_{t,i} a_{t,i} \\
F_{\ast,t,i} &=& Z_{t,i} P_{\ast,t,i} Z_{t,i}' + \sigma^2_{t,i} \\
F_{\infty,t,i} &=& Z_{t,i} P_{\infty,t,i} Z_{t,i}' \\
K_{\ast,t,i} &=& P_{\ast,t,i} Z_{t,i}' \\
K_{\infty,t,i} &=& P_{\infty,t,i} Z_{t,i}',
\end{eqnarray*}
and
\begin{eqnarray*}
a_{t,i+1} &=& a_{t,i} + K_{\infty,t,i} v_{t,i}F_{\infty,t,i}^{-1} \\
P_{\ast,t,i+1} &=& P_{\ast,t,i}
+K_{\infty,t,i}K'_{\infty,t,i}F_{\ast,t,i}F_{\infty,t,i}^{-2}
-(K_{\ast,t,i}K'_{\infty,t,i}+K_{\ast,t,i}K'_{\infty,t,i})F_{\infty,t,i}^{-1}\\
P_{\infty,t,i+1} &=& P_{\infty,t,i}
-K_{\infty,t,i}K'_{\infty,t,i}F_{\infty,t,i}^{-1}\\
\end{eqnarray*}
if $F_{\infty,t,i}>0$, and
\begin{eqnarray*}
a_{t,i+1} &=& a_{t,i} + K_{\ast,t,i} v_{t,i}F_{\ast,t,i}^{-1} \\
P_{\ast,t,i+1} &=& P_{\ast,t,i} -K_{\ast,t,i}K'_{\ast,t,i}F_{\ast,t,i}^{-1}\\
P_{\infty,t,i+1} &=& P_{\infty,t,i},\\
\end{eqnarray*}
if $F_{\infty,t,i}=0$. The transition equations from $t$ to $t+1$ are
\begin{eqnarray*}
a_{t+1,1} &=& T_t a_{t,p_t+1}\\
P_{\ast,t+1,1} &=& T_t P_{\ast,t,p_t+1} T_t' + R_t Q_t R_t\\
P_{\infty,t+1,1} &=& T_t P_{\infty,t,p_t+1} T_t'. 
\end{eqnarray*}
...
\begin{eqnarray*}
r_{t,i-1} &=& Z'_{t,i}v_{t,i}F_{t,i}^{-1} + L'_{t,i}r_{t,i}, \\
r_{t-1,p_t} &=& T'_{t-1} r_{t,0},\\
N_{t,i-1} &=& Z_{t,i}'Z_{t,i}F_{t,i}^{-1} +L_{t,i}'N_{t,i}L_{t,i},\\
N_{t-1,p_t} &=& T'_{t-1}N_{t,0}T_{t-1},\\
L_{t,i} &=& I - K_{t,i}Z'_{t,i}F_{t,i}^{-1},
\end{eqnarray*}
...
\begin{eqnarray*}
\hat \epsilon_{t,i} &=& \sigma^2_{t,i}F_{t,i}^{-1}(v_{t,i} -K'_{t,i}r_{t,i}),\\
\VAR(\hat \epsilon_{t,i}) &=& \sigma^2_{t,i} -
\sigma^4_{t,i}(F_{t,i}^{-1} -K'_{t,i}N_{t,i}K_{t,i}F_{t,i}^{-2}),\\
\hat \eta_{t} &=& Q_tR'_tr_{t,0},\\
\VAR(\hat \eta_{t,i}) &=& Q_tR'_tN_{t,0}R_tQ_t.
\end{eqnarray*}
...
r_{0,t,i-1} &=& L'_{\infty,t,i}r_{0,t,i}, \\
r_{1,t,i-1} &=& Z'_{t,i}v_{t,i}F_{\infty,t,i}^{-1} + L'_{\infty,t,i}r_{1,t,i} +
L'_{t,i}r_{0,t,i}, \\
N_{0,t,i-1} &=& L_{\infty,t,i}'N_{0,t,i}L_{\infty,t,i}\\
N_{1,t,i-1} &=& L_{t,i}'N_{0,t,i}L_{\infty,t,i}+
L_{\infty,t,i}'N_{1,t,i}L_{\infty,t,i}+Z_{t,i}'Z_{t,i}F_{\infty,t,i}^{-1},\\
N_{2,t,i-1} &=& L_{t,i}'N_{0,t,i}L_{t,i}+ L_{\infty,t,i}'N_{1,t,i}L_{t,i} +
L_{t,i}'N_{1,t,i}L_{\infty,t,i} +
L_{\infty,t,i}'N_{2,t,i}L_{\infty,t,i}-Z_{t,i}'Z_{t,i}F_{t,i}F_{\infty,t,i}^{-2},\\
N_{t-1,p_t} &=& T'_{t-1}N_{t,0}T_{t-1},
\end{eqnarray*}
if $F_{\infty,t,i}>0$, and
\begin{eqnarray*}
L_{t,i} &=& I - K_{t,i}Z_{t,i}F_{t,i}^{-1},\\
r_{0,t,i-1} &=& Z'_{t,i}v_{t,i}F_{t,i}^{-1} + L'_{t,i}r_{0,t,i}, \\
r_{1,t,i-1} &=& L'_{t,i}r_{1,t,i}, \\
N_{0,t,i-1} &=& L_{t,i}'N_{0,t,i}L_{t,i}+Z_{t,i}'Z_{t,i}F_{t,i}^{-1}\\
N_{1,t,i-1} &=& N_{1,t,i}L_{t,i}\\
N_{2,t,i-1} &=& N_{2,t,i}L_{t,i},
\end{eqnarray*}
otherwise. The transition from time $t$ to $t-1$ is by $N_{j,t-1,p_t} =
T'_{t-1}N_{j,t,0}T_{t-1}$ for $j=0,1,2$, and $r_{j,t-1,p_t} = T'_{t-1}r_{j,t,0}$
...
V_{t} &=& P_{t,1}-P_{t,1}N_{0,t,0}P_{t,1} -(P_{\infty,t,1}N_{1,t,0}P_{t,1})'
-P_{\infty,t,1}N_{1,t,0}P_{t,1} - P_{\infty,t,1}N_{2,t,0}P_{\infty,t,1},
\end{eqnarray*} 
and disturbance smoothing recursions
\begin{eqnarray*}
\hat \epsilon_{t,i} &=& -\sigma^2_{t,i}K'_{\infty,t,i}r_{0,t,i},\\
\VAR(\hat \epsilon_{t,i}) &=& \sigma^2_{t,i} -
\sigma^4_{t,i}K'_{\infty,t,i}N_{0,t,i}K_{\infty,t,i}F_{\infty,t,i}^{-2},
\end{eqnarray*}
if $F_{\infty,t,i}>0$, and
\begin{eqnarray*}
\hat \epsilon_{t,i} &=&
-\sigma^2_{t,i}(v_{t,i}F_{\infty,t,i}^{-1}-K'_{t,i}r_{0,t,i}),\\
\VAR(\hat \epsilon_{t,i}) &=& \sigma^2_{t,i} -
\sigma^4_{t,i}(F_{t,i}^{-1}-K'_{t,i}N_{0,t,i}K_{t,i}F_{t,i}^{-2}),
\end{eqnarray*}
if $F_{\infty,t,i}=0$. For $\hat\eta$, recursions are
\begin{eqnarray*}
\hat \eta_{t} &=& Q_tR'_tr_{0,t,0},\\
\VAR(\hat \eta_{t,i}) &=& Q_tR'_tN_{0,t,0}R_tQ_t.
\end{eqnarray*}
