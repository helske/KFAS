...
simulateSSM <- 
  function(object, type = c("states", "signals", "disturbances", "observations", 
    ...
    dfeps <- sum(x)/nsim
    x2 <- array(abs(apply(object$Q, 3, diag)) > object$tol, c(k, (n - 1) * tv[5] + 
        1))
    ...
    if (out$info != 0) {
        if (out$info == -2) 
            stop("Couldn't compute LDL decomposition of Q.")
        if (out$info == -3) 
            stop("Couldn't compute LDL decomposition of P1.")
    }
    rownames(out$sim) <- switch(sim.what, rep("eps", p), rep("eta", k), c(rep("eps", 
        p), rep("eta", k)), rownames(object$a1), colnames(object$y), colnames(object$y))
