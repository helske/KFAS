...
#'
#' Function \code{is.SSModel} tests whether the object is a valid 
#' \code{SSModel} object.
#'
...
#' @param na.check Test the system matrices for NA and infinite values. 
#' Default is \code{FALSE}.
#' @param return.logical If \code{FALSE}, error is given if the the model is 
#' not a valid \code{SSModel} object. Otherwise logical value is returned. 
#' Default is \code{FALSE}.
  ...
  function(object, na.check = FALSE, return.logical = TRUE) {
    ...
    if (return.logical) {
      x <- inherits(object, "SSModel") && 
        ...
        (object$H == "Omitted" || identical(dim(object$H), c(p, p, one)) ||
        ...
        (object$u == "Omitted" || identical(dim(object$u), dim(object$y)))
      if (na.check) 
        x <- x && 
        !any(sapply(c("H", "u", "T", "R", "Q", "a1", "P1", "P1inf"), function(x)
          any(is.na(object[[x]])) || any(is.infinite(object[[x]])))) && 
        max(object$Q)<=1e10 && ifelse(object$u == "Omitted", max(object$H)<=1e10,TRUE)
      x
    } else {
      ...
      if (na.check == TRUE && 
        ...
        max(object$Q)<=1e10 && ifelse(object$u == "Omitted", max(object$H)<=1e10,TRUE))
        stop("System matrices (excluding Z) contain NA or infinite values, or covariance matrices contain values larger than 1e10.")
