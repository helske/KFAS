...
interval <- 
  function(model, interval = c("confidence", "prediction"), level, 
    ...
    if (type == "link") {
      ...
      for (j in 1:p) {
        imp$samples[timespan, j, ] <- 
          switch(model$distribution[j], 
                 gaussian = imp$samples[timespan, j, ], 
                 poisson = exp(imp$samples[timespan, j, ]), 
                 binomial = (if (!prob) model$u[timespan, j] else 1) * 
                   exp(imp$samples[timespan, j, ])/(1 + exp(imp$samples[timespan, j, ])), 
                 gamma = exp(imp$samples[timespan, j, ]), 
                 `negative binomial` = exp(imp$samples[timespan, j, ]))
      }
