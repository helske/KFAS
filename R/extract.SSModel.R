...
#' @param element Which element is chosen. Possible choices are 
#' 'y','Z','H','T','R','Q','a1','P1','P1inf', and 'u'.
#' @param states Which states are chosen. Either a numeric vector containing 
#' the indices of the states, or a character vector defining the 
#' types of the states. 
#' Possible choices are \dQuote{all}, \dQuote{arima}, \dQuote{custom}, 
#' \dQuote{cycle}, \dQuote{seasonal}, \dQuote{trend}, or \dQuote{regression}. 
#' These can be combined. Default is \dQuote{all}.
#' @param etas Which disturbances eta are chosen. Used for elements \dQuote{R} 
#' and \dQuote{Q}. Either a numeric vector containing the indices of the 
#' etas, or a character vector defining the types of the etas. 
#' Possible choices are \dQuote{all}, \dQuote{arima}, \dQuote{custom}, 
#' \dQuote{cycle}, \dQuote{seasonal}, \dQuote{trend}, or \dQuote{regression}. 
#' These can be combined. 
#' @param series Numeric. Which series are chosen. Used for elements 
#' \dQuote{y}, \dQuote{Z}, and \dQuote{u}.
...
`[.SSModel` <- 
  function(x, element, states, etas, series, times, drop=TRUE,...) {
    ...
    if (missing(times)) {
      switch(element, y = , u = x[[element]][, series, drop = drop], Z = x[[element]][series, 
                                                                                       states, , drop = drop], H = x[[element]][series, series, , drop = drop], 
             T = x[[element]][states, states, , drop = drop], R = x[[element]][states, 
                                                                                etas, , drop = drop], Q = x[[element]][etas, etas, , drop = drop], 
             a1 = x[[element]][states, 1, drop = drop], P1 = x[[element]][states, 
                                                                           states, drop = drop], P1inf = x[[element]][states, states, drop = drop], 
      )
    } else {
      switch(element, y = , u = x[[element]][times, series, drop = drop], Z = x[[element]][series, 
                                                                                            states, times, drop = drop], H = x[[element]][series, series, times, 
                                                                                                                                           drop = drop], T = x[[element]][states, states, times, drop = drop], 
             R = x[[element]][states, etas, times, drop = drop], Q = x[[element]][etas, 
                                                                                   etas, times, drop = drop], a1 = x[[element]][states, 1, drop = drop], 
             P1 = x[[element]][states, states, drop = drop], P1inf = x[[element]][states, 
                                                                                   states, drop = drop], )
    }
  } 
