...
#' \eqn{v_{t,i}=y_{t_i}-Z_{t_i}a_{t_i}}{v[t,i]=y[t_i]-Z[t_i]a[t_i]},
...
#' 
#' \item 'pearson':  \deqn{(y_{t,i}-\theta_{t,i})/\sqrt{V(\mu_{t,i})}, \quad i=1,\ldots,p,t=1,\ldots,n,}{(y[t,i]-\theta[t,i])V(\mu[t,i])^(-0.5), i=1,\ldots,p, t=1,\ldots,n,}
...
#' @param ... Ignored.
residuals.KFS <- 
  function(object, type = c("recursive", "deviance", "pearson", "response", 
                            "state"), ...) {
    type <- match.arg(type)
    ...
    series <- switch(type, 
                     recursive = {
                       if (is.null(object[["v", exact = TRUE]])) 
                         stop("KFS object needs to be run with state filtering. ")
                       series <- object$v
                       if (object$d > 0) {
                     ...
                     pearson = {
                       series <- object$model$y
                       if (sum(bins <- object$model$distribution == "binomial") > 0) series[, bins] <- series[, 
                                                                                                              bins]/object$model$u[, bins]
                       ...
                       for (i in 1:attr(object$model, "p")) series[, i] <- ifelse(series[, i] > 
                                                                                    object$muhat[, i], 1, -1) * sqrt(switch(object$model$distribution[i], 
                                                                                                                            gaussian = (series[, i] - object$muhat[, i])^2, poisson = 2 * (series[, 
                                                                                                                              ...
                                                                                                                              (log(ifelse(series[, i] == 0, 1, series[, i]/object$muhat[, i])) - 
                                                                                                                                 (series[, i] - object$muhat[, i])/object$muhat[, i]), `negative binomial` = 2 * 
                                                                                                                              (series[, i] * log(pmax(1, series[, i])/object$muhat[, i]) - (series[, 
                                                                                                                                                                                                   i] + object$model$u[, i]) * log((series[, i] + object$model$u[, 
